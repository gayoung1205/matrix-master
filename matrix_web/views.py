...
def home(req):
    ...
    context = {'matrix': matrix, 'profile': profile, 'main': main, 'connect_list': connect, 'permission': req.user.userpermission.permission}
...
def profile_template(req):
    ...
    mat = Mat.objects.filter(is_main=False)
    matrix = mat.order_by("created_date")
    mat_connect = mat.order_by("main_connect")
    main = Mat.objects.filter(is_main=True)
    profile = Profile.objects.all().order_by('id')

    connect = ['입력01', '입력02', '입력03', '입력04']
    for i in range(0, len(mat_connect)):
        connect[i] = mat_connect[i].name
   
    context = {'matrix': matrix, 'profile': profile, 'main': main, 'connect_list': connect}
